...
def run_episode(env, agent, rpm):
    ...
    #w = [0.0564,0.1073,0.1,0.1295,0.14,0.4668]
    flag1 -=1
    ...
    while True:
        ...
        next_obs, cost, done = env.step(action)
        ...
        #print("第",step,"次step的reward为",reward)
        #rpm.append((obs, action, reward, next_obs, done))
        # train model
        # 当经验池放够数据，才训练模型
        if (len(rpm) > MEMORY_WARMUP_SIZE) and (step % LEARN_FREQ == 0):
            ...
            #print("batch_action is",batch_action,len(batch_action))
...
def evaluate(env, agent):
    ...
    return eval_totalCost,eval_totalReward
...
def main():
    ...
    while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
        ...
        # test part
        eval_totalCost,eval_totalReward = evaluate(env, agent) 
        root_logger = logging.getLogger()
        for h in root_logger.handlers[:]:
            root_logger.removeHandler(h)
        ...
        #print('episode:{}    e_greed:{}  average Cost: {} average Reward:{} Test Cost:{} Reward:{} Action:{}  State:{} '.format(
        #    episode, agent.e_greed,np.mean(eval_totalCost),np.mean(eval_totalReward), eval_totalCost, eval_totalReward ,env.action_queue, env.State))
